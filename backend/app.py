...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'sqlite:///musicapp.db')
    ...
    def root():
        return jsonify({
            'message': 'Music App Backend API is running!',
            'status': 'success',
            'version': '1.0.0',
            'endpoints': {
                'health': '/api/health',
            ...
            'websocket': {
                'events': ['connect', 'disconnect', 'join_room', 'leave_room', 'send_message', 'typing'],
                'namespace': '/'
            }
        })
